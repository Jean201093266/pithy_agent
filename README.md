...
- `/api/chat` 现支持 `session_id`，用于会话隔离与跨会话记忆复用。
- 记忆工作流采用 `Retrieve -> Reason -> Update`：
  - `Retrieve`：短期记忆（滑动窗口 + 摘要 + 结构化状态）+ 长期记忆召回
  - `Reason`：将记忆上下文注入 ReAct 推理与最终回答
  - `Update`：写入情节记忆/事实记忆/偏好记忆，并执行容量裁剪
- 已引入 `LangChain + LangGraph` 作为成熟编排框架：
  - `app/core/langchain_adapter.py`：将现有 LLM 与工具注册表桥接到 LangChain 运行时
  - `app/core/chat_graph.py`：图式节点编排（retrieve/reason/update）
- 若运行环境缺少相关依赖或图执行异常，系统会自动回退到 legacy 路径，保证接口可用。
